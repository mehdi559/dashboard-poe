...
export class ExcelExportEngine {
  ...
  static createDashboardSheet(data, language) {
    ...
    const savingsProgress = totalTarget > 0 ? (totalSavings / totalTarget * 100).toFixed(1) : '0.0';
    
    const totalDebt = debts ? debts.reduce((sum, d) => sum + d.balance, 0) : 0;
    const debtToIncomeRatio = monthlyIncome > 0 ? (totalDebt / monthlyIncome * 100).toFixed(1) : '0.0';
    ...
    const daysUntilBudgetExhaustion = totalBudget > 0 ? Math.floor((totalBudget - totalSpent) / averageDailySpending) : 0;
    ...
    const worksheetData = [
      ...
      ['', t.savingsRate, `${savingsRate ? savingsRate.toFixed(1) : (monthlyIncome > 0 ? ((monthlyIncome - totalSpent) / monthlyIncome * 100).toFixed(1) : '0.0')}%`, 
       '20%', `${((savingsRate || (monthlyIncome > 0 ? ((monthlyIncome - totalSpent) / monthlyIncome * 100) : 0)) - 20).toFixed(1)}%`, 
       (savingsRate || (monthlyIncome > 0 ? ((monthlyIncome - totalSpent) / monthlyIncome * 100) : 0)) > 20 ? t.excellent : t.needsImprovement, '', ''],
      ['', t.budgetUtilization, `${totalBudget > 0 ? (totalSpent / totalBudget * 100).toFixed(1) : '0.0'}%`, 
       '80%', `${totalBudget > 0 ? ((totalSpent / totalBudget * 100) - 80).toFixed(1) : '-80.0'}%`, 
       totalBudget > 0 && totalSpent / totalBudget > 0.9 ? t.warning : t.good, '', ''],
      ['', '', '', '', '', '', '', ''],
    ...
    categories.forEach(category => {
      ...
      const percentage = category.budget > 0 ? (spent / category.budget * 100).toFixed(1) : '0.0';
    ...
    worksheetData.push(
      ...
      ['', t.averageDailySpending, `${averageDailySpending.toFixed(2)} ${t.currency}`, '', '', '', '', ''],
      ['', t.weeklyAverage, `${this.calculateWeeklyAverage(expenses).toFixed(2)} ${t.currency}`, '', '', '', '', ''],
      ['', '', '', '', '', '', '', ''],
      ['', `💡 ${t.recommendations}`, '', '', '', '', '', ''],
      ['', '', '', '', '', '', '', '']
    );
  ...
  static createAnalysisSheet(data, language) {
    ...
    dailyAnalysis.forEach((dayData, index) => {
      worksheetData.push([
        ...
        dayData.count > 0 ? (dayData.total / dayData.count).toFixed(2) : 0, '', '', ''
      ]);
    });
    ...
    categories.forEach(category => {
      ...
      worksheetData.push([
        '', category.name, total, count, avg.toFixed(2), `${percentage}%`, '', ''
      ]);
    });
  ...
  static createChartsSheet(data, language) {
    ...
    categories.forEach(category => {
      const categoryTotal = expenses
        .filter(e => e.category === category.name)
        .reduce((sum, e) => sum + e.amount, 0);
      const percentage = totalSpent > 0 ? (categoryTotal / totalSpent * 100).toFixed(1) : 0;
    ...
    weeklyData.forEach((weekData, index) => {
      ...
      worksheetData.push([
        '', `${t.week} ${index + 1}`, weekData.spent.toFixed(2), weekData.budget.toFixed(2), 
        weekData.variance.toFixed(2), status, '', ''
      ]);
    });
  ...
  static calculateWeeklyAverage(expenses) {
    const total = expenses.reduce((sum, e) => sum + e.amount, 0);
    return total > 0 ? (total / 4).toFixed(2) : '0.00'; // Protection contre division par zéro
  }

  static calculateDailyAverage(expenses) {
    const total = expenses.reduce((sum, e) => sum + e.amount, 0);
    const daysWithExpenses = new Set(expenses.map(e => e.date)).size;
    return daysWithExpenses > 0 ? (total / daysWithExpenses).toFixed(2) : '0';
  }
  ...
  static generateAIRecommendations(data) {
    ...
    if (actualSavingsRate < 20) {
      recommendations.push({
        ...
        potentialSaving: `${(totalSpent * 0.1).toFixed(2)}€`
      });
    }
