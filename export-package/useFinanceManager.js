...
const useFinanceManager = () => {
  ...
  useEffect(() => {
    ...
    // Charger spécifiquement le nom d'utilisateur depuis localStorage
    const savedUserName = localStorage.getItem('userName');
    if (savedUserName && savedUserName.trim()) {
      dispatch({ type: ACTIONS.SET_USER_NAME, payload: savedUserName.trim() });
    }
  }, []);
  ...
  const actions = {
    ...
    exportData: () => {
      ...
      const filename = dataUtils.exportToJSON(exportData);
      showNotification(`✅ Données exportées avec succès ! Fichier: ${filename}`);
    },

    exportExpensesToCSV: () => {
      const filename = dataUtils.exportToCSV(state.expenses, 'expenses');
      showNotification(`✅ Dépenses exportées en CSV ! Fichier: ${filename}`);
    },
